var NeatoLib = {
    ...
    hasRequiredLibVersion: function(plugin, requiredVersion)
    {
        if(NeatoLib.parseVersion(NeatoLib.version).compareTo(NeatoLib.parseVersion(requiredVersion)) == "older")
        {
            ...
            {
                const vm = require("vm");
                fetch("https://raw.githubusercontent.com/Metalloriff/BetterDiscordPlugins/master/Lib/NeatoBurritoLibrary.js",
                {
                    cache: "no-cache"
    ...
    Settings:
    {
        ...
        {
            const panel = document.getElementById(`plugin-settings-${plugin.getName()}`),
                fields = plugin.settingFields;
            ...
            { //<input id="1" class="checkboxEnabled-CtinEn checkbox-2tyjJg da-checkboxEnabled da-checkbox" type="checkbox" tabindex="-1">
            ...
            for(let setting in fields)
            {
                ...
                try
                {
                    ...
                    if(field.array || field.list)
                    {
                        ...
                        if(field.array)
                        {
                            ...
                            addButton.addEventListener("click", e =>
                            {
                                ...
                                array.innerHTML = "";
                                repop();
                                if(typeof field.callback == "function") field.callback(
                                {
                                    type: "add",
                                    event: e,
                                    array: array,
                            ...
                            insertDeleteButton = i =>
                            {
                                ...
                                NeatoLib.Tooltip.attach("Remove", button,
                                {
                                    delay: 250
                                });
                    ...
                    const set = (nv, e) =>
                    {
                        ...
                        {
                            type: "change",
                            oldValue: plugin.settings[setting],
                            newValue: nv,
                            event: e,
                            panel: panel
                        });
                                ...
                                (repop = () =>
                                {
                                    for(let i in plugin.settings[setting])
                                    {
                                        const element = createToggleSwitch(null, null, null, plugin.settings[setting][i], (nv, e) =>
                                        {
                                            ...
                                            {
                                                type: "change",
                                                oldValue: plugin.settings[setting][i],
                                                newValue: nv,
                                                event: e,
                                                panel: panel
                                            });
    ...
    Modules:
    { //Based off of Zerebos' PluginLibrary. https://rauenzi.github.io/BetterDiscordAddons/docs/PluginLibrary.js
    ...
    Data:
    {
        ...
        load: function(name, key, fallback)
        {
            try
            {
                return $.extend(true, fallback ? fallback :
    ...
    Tooltip:
    {
        attach: function(content, element, options = {})
        {
            ...
            const
            {
                side = "top", color, onShow, onHide, delay
            } = options;
...
var mesquite = BdApi.Plugins.getAll().map(x => x.name);
/*
